...
export async function GET() {
  ...
  try {
    ...
    if (teacher) {
      ...
      assignedClasses = myClasses.map(c => ({
        id: c.kelasId,
    ...
    if (assignedClasses.length > 0) {
       ...
       // we can just fetch all attendance for these classes
       ...
       const filteredAtt = allAtt.filter(a => classNames.includes((a as any).kelas || ""));
       // Actually innerJoin returns flat or object depending on select. My select above only gets attendance status.
