...
export default function SiswaLeaderboardPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch("/api/spk/calculate?kelas=umum").then(r => r.json()),
      fetch("/api/students/me").then(r => r.json())
    ]).then(([spkRes, profileRes]) => {
      if (Array.isArray(spkRes)) setData(spkRes);
      if (profileRes && !profileRes.error) {
        setMyData({
           nama: profileRes.namaLengkap,
           kelas: profileRes.kelas,
           studentId: profileRes.id
        });
      }
      setLoading(false);
                ...
                {data.map((entry) => (
                  ...
                  >
                    ...
                    <TableCell className="text-right pr-6">
                      <span className="font-medium text-muted-foreground text-xs">{entry.rawScore.toFixed(4)}</span>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
